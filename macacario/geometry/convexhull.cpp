...
vector<point> convexHull(vector<point> P) {
	...
	while (i < n) {
		j = (int)S.size()-1;
		if (ccw(S[j-1], S[j], P[i])) S.push_back(P[i++]);	//Nao inclui pontos colineares
		//if (ccw(S[j-1], S[j], P[i]) || collinear(S[j-1], S[j], P[i])) S.push_back(P[i++]);	//Inclui pontos colineares
		else S.pop_back();
	}
	return S;
}
