...
public class kingdom_mb_fast {
    ...
    static class Edge {
        ...
        public Edge link(Edge that) {
            Edge rv = new Edge(source, that.source);
            Edge tw = rv.twin;
            rv.next = that;
            rv.prev = prev;
            tw.next = this;
    ...
    static class Out {
        ...
        private StringBuilder sb = new StringBuilder(60);

        void append(double v, StringBuilder res) {
            if (v < 0) {
                sb.append('-');
                v = -v;
            }
        ...
        public void addTo(StringBuilder res) {
            if (sb.length() == 0) {
                append(x, sb);
                sb.append(" ");
                append(y, sb);
            ...
            if (curr.next == first) {
                ...
                while (first.source != last.source) {
                    if (curr.next == last && curr.prev.prev == last) {
                        ...
                        first = first.prev.link(first.next);
                        allEdges.add(first);
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; ++i) {
            Edge inner = pts[i].innerEdge;
            inner.next = inner.target.innerEdge;
            inner.next.prev = inner;
            inner.next.twin.next = inner.twin;
            inner.twin.prev = inner.next.twin;
        }
            ...
            switch (curr.type) {
                ...
                case END: {
                    ...
                    if (prev.helper.type == Type.MERGE) {
                        Edge e = curr.innerEdge.link(prev.helper.innerEdge);
                        prev.helper.innerEdge = e.twin;
                        allEdges.add(e);
                        allEdges.add(e.twin);
                    }
                    edges.remove(prev);
                ...
                case SPLIT: {
                    ...
                    Edge e;
                    if (left.helper.type == Type.SPLIT && left.helper.innerEdge.source.vmulAround(left.helper.innerEdge.target, curr) > 0) {
                        e = curr.innerEdge.link(left.helper.splitEdge);
                    } else {
                        e = curr.innerEdge.link(left.helper.innerEdge);
                    }
                    allEdges.add(e);
                    allEdges.add(e.twin);
                    left.helper = curr;
                ...
                case MERGE: {
                    ...
                    if (left.helper.type == Type.MERGE) {
                        Edge e = curr.innerEdge.link(left.helper.innerEdge);
                        curr.innerEdge = e;
                        allEdges.add(e);
                        allEdges.add(e.twin);
                    }
